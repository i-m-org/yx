...
export default function HistoricoPage() {
  ...
  // Generar opciones de meses (ultimos 24 meses)
  ...
  // Combinar registros historicos con datos calculados del mes actual
  const todosLosRegistros = useMemo(() => {
    // Obtener todos los meses unicos de metas y ventas
    ...
    // Para cada combinacion de mes/sucursal/producto
    Array.from(mesesUnicos).forEach((mes) => {
      sucursales.forEach((sucursal) => {
        productos.forEach((producto) => {
          // Buscar si existe un registro historico
  ...
  // Agrupar por mes para la grafica
    ...
    <MainLayout>
      <div className="border-b border-border bg-card/50 px-4 py-4 sm:px-6 lg:px-8 lg:py-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-xl font-bold text-foreground sm:text-2xl">Historico</h1>
                    ...
                    <TableRow key={registro.id}>
                      ...
                      <TableCell>
                        <Badge variant={registro.esHistorico ? "default" : "outline"}>
                          {registro.esHistorico ? "Historico" : "Calculado"}
                        </Badge>
                      </TableCell>
                    </TableRow>
                  ))}
